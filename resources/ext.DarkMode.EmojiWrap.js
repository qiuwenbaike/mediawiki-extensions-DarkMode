...
( () => {
	...
	for ( const node of nodeArray ) {
		...
		if ( emojiRegExpMatchArray && !( hasClass( 'action-edit' ) || hasClass( 'action-submit' ) ) ) {
			const surroundEmoji = function ( emoji ) {
				const self = this;
				const { nodeValue: _nodeValue } = self;
				if ( !_nodeValue ) {
					return;
				}
				emoji ??= _nodeValue;
				...
				range.setStart( self, startIndex );
				range.setEnd( self, startIndex + emoji.length );
