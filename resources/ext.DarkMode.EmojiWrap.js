...
( () => {
	...
	const surroundEmojiText = ( node, nodeValue ) => {
		...
		const TAG_NAME = 'qiuwen-emoji';
		const surroundEmojis = function ( emoji ) {
		...
		const recursiveNodeNextSibling = ( _node ) => {
			...
			if ( emojiRegExpMatchArrayInside ) {
				_node.surroundEmojis( emojiRegExpMatchArrayInside[ 0 ] );
			}
			recursiveNodeNextSibling( nextSibling );
		};
		Text.prototype.surroundEmojis = surroundEmojis;
		for ( const emoji of emojiRegExpMatchArray ) {
			...
			if (
				...
				nodeName.includes( TAG_NAME.toUpperCase() )
			) {
				continue;
			}
			node.surroundEmojis( emoji );
			recursiveNodeNextSibling( node );
		}
		delete Text.prototype.surroundEmojis;
	};

	const targetTextNodeArray = getTextNodeArray( document.body );
